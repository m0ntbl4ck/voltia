...
| ¿Qué lecturas se salen de lo esperado? | Baseline horario robusto + detectores D1–D7 | Medidores / Detalle |
| ¿Real, explicable o calidad de datos? | Clasificador determinista + correlación con eventos + chequeo físico | Anomalías IA |
| ¿Cuál investigar primero? | Severidad + score de prioridad 0–100 | Anomalías IA / Dashboard |
...
| Aspecto | Decisión |
|---|---|
| Plazo | 4 días (24–27 sep 2026), 1 desarrollador, desarrollo asistido por IA |
...
│   ├── analysis/                  # ⭐ motor puro y testeable
...
| Calidad | `golangci-lint` · ESLint + Prettier · GitHub Actions · Makefile · skills **antislop** (ver §11.1) |
...
| D2 | Spike / caída brusca | racha de 1–5 h con \|z\| > 4 que vuelve a lo normal |
...
|---|---|---|---|
| `REAL_ANOMALY` | variación ≥ 50% **o** señales D5 | 20–50% | < 20% |
| `DATA_QUALITY` | persistente (≥ 5 lecturas inválidas o en curso) | 2–4 lecturas | 1 aislada |
| `EXPLAINABLE_ANOMALY` | — | variación ≥ 20% | < 20% |
| `FALSE_POSITIVE` | — | — | siempre |

### 5.6 Prioridad (0–100, para ordenar)

```
prioridad = severidad (HIGH 50 · MEDIUM 25 · LOW 5)
          + tipo      (REAL 25 · DATA_QUALITY 15 · EXPLAINABLE 5 · FP 0)
          + impacto   (0–15, proporcional a kWh en exceso)
...
Acotado a **[0,50 – 0,99]**. Bandas: Alta ≥ 0,85 · Media/Alta 0,75–0,85 · Media 0,60–0,75 · Baja < 0,60.
...
|---|---|---|
| 1 ⭐ | **Regresión sobre el dataset real**: los 4 casos + 8 sanos + inicio de episodio de M-109 | `go test` |
| 2 | Unitarios del motor: baseline, D1–D7, clasificador y reglas de seguridad, scoring, determinismo IF, guarda de números | `go test` table-driven |
| 3 | API: códigos, filtros, auth, RFC 7807 | `httptest` + repos fake |
| 3+ | 3–4 tests críticos contra Postgres real: filtros/orden, seed idempotente, dedupe por fingerprint | Testcontainers |
...
### 11.1 Guías de calidad con antislop (desarrollo asistido por IA)

Como el desarrollo es asistido por IA, se usa el plugin **antislop** (skills de Claude Code) para evitar el resultado genérico típico de IA en interfaz, textos y código. Su uso es obligatorio en las fases indicadas:

| Skill | Se aplica a | Cuándo |
|---|---|---|
| `antislop` (núcleo) | Todo el trabajo: siempre activo | Todo el proyecto |
| `antislop-ui` | Paleta, layout, componentes, animación de las 7 pantallas | Día 3 y pulido del día 4 |
| `antislop-human` | Contraste (con el verificador del plugin), navegación por teclado, foco visible y estados vacío/carga/error | Al definir la paleta del tema oscuro y claro (colores de severidad incluidos) y antes de grabar la demo |
| `antislop-copywriting` | Textos de la UI (es-CO), README, guion de la demo y **prompt de Gemini + plantillas de explicación** | Día 2 (prompt y plantillas), día 3 (UI) y día 4 (README) |
| `antislop-code` | Comentarios del código Go y TS: quitar los genéricos y conservar los que explican el porqué | Antes de cada PR |
| `antislop-layoutmobile` | Adaptación a tablet y móvil (secundaria: la prioridad es escritorio) | Solo si sobra tiempo |
...
- El flujo de la demo se puede recorrer con teclado y el foco es visible.
...
- Ningún PR añade comentarios que solo repiten lo que hace el código.
...
| **Vie 25** | La API y la IA hablan | API completa + OpenAPI; Explainer (plantillas → Gemini) + guarda + caché; Isolation Forest; prompt y plantillas revisados con `antislop-copywriting` |
| **Sáb 26** | Voltia se ve como producto | Las 7 pantallas y el flujo de la demo completo con `docker compose up`, construidas con `antislop-ui` + `antislop-human` (contraste verificado) |
| **Dom 27** | Entrega | Pulido, tests restantes, adaptador Claude, README + ADRs (`antislop-copywriting`), pasada de `antislop-code`, prueba en limpio sin key, **grabación de la demo** |
...
## 13. Guion de demo (grabada, 5–10 min)
