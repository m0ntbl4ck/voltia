# ADR 0011: Despliegue en una instancia EC2 con Docker Compose y Caddy
...
- Caddy consigue el certificado con Let's Encrypt para un nombre `A-B-C-D.sslip.io` que apunta a una IP elástica, sirve el frontend compilado y reenvía `/api` a la API. El navegador ve un solo origen, así que no hace falta CORS y la cookie `SameSite=Lax` funciona.
...
- Frontend en Netlify o Amplify con reescritura de `/api`: añade un proveedor y depende de que el proxy conserve las cookies. Con Caddy en la misma instancia no se necesita.
...
- Una sola máquina: si cae, cae todo. Los datos están en un volumen del disco de la instancia y no hay copias de seguridad automáticas.
