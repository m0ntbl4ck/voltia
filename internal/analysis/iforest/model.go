...
// ModelConfig adds the score that counts as unusual to the forest settings.
type ModelConfig struct {
	Forest Config
	// Threshold is the score from which a reading backs up an episode. A
	// reading the forest finds ordinary scores around 0.5 or below.
	Threshold float64
}

func DefaultModelConfig() ModelConfig {
	return ModelConfig{Forest: DefaultConfig(), Threshold: 0.6}
}
...
// Corroborate scores the readings that fall inside the episode. When any
// reaches the threshold it returns the signal that backs the episode up: it
// spans the first to the last unusual hour, Hours counts them, Observed is the
// highest score, Expected the threshold, and Attribution and Variable say
// which variables isolated them. It never creates an episode of its own.
